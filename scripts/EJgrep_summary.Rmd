...
```{r echo=FALSE}
filteredTbl <- freak_count_loader("analysis/alleleCounts.simpleIndels.dpthFilt.biallelic.universal.out") 
...
Since DSB is likely to create unique variants in the population, 

```{r echo=FALSE}
ggplot(primitive_tubby.biallele) + geom_freqpoly(aes(x=mac, color=chrom),bins=31)  + theme_clear() + labs(x="Minor Allele Count (chromosomes)", y="Number of Sites", title="Number of Quality INDELs, by Minor Allele Count and Chromosome")
```
