...
```{r echo=FALSE}
data_sets.df.sparse %>% filter(experimental == 'control') %>% select(c(name, source, pedigree, sex)) %>%  arrange(pedigree, desc(name)) %>% kable(caption="Sequenced Samples from Control Cross")
```
...
To build this VCF, `r nrow(data_sets.df.sparse)` samples called jointly. However, not all sites were called in all samples (eg, due to coverage differences). The sites had the following group-wide call rate:
...
nsamps <- nrow(data_sets.df.sparse)
...
_important for comparing variant-calling methods, but not for comparing unrelated sample groups - they get called individually!_
