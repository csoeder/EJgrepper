...
cat indelCount.test.frq.count  | tail -n +2 | tr ":" "\t" | nl -n ln | head -n 1000 > dev/indelCount.test.frq.count
...
max_fields<- count.fields("indelCount.test.frq.count", sep = '\t') %>% max()


tubble <- read.table("indelCount.test.frq.count", header = FALSE, sep = "\t", fill = TRUE,col.names = paste0("V",seq_len(max_fields)))

tubble.df <- tubble[1:7]
names(tubble.df) <- c("site_id", "chrom", "pos", "var_count", "chroms_avail", "allele", "allele_count")
tubble.df$var_num <- 1

for (i in seq(8,max_fields,2)){
tubble.df.tmp <- cbind(tubble[1:5], tubble[c(i,i+1)])
names(tubble.df.tmp) <- c("site_id", "chrom", "pos", "var_count", "chroms_avail", "allele", "allele_count")
tubble.df.tmp$var_num <- i/2-2
tubble.df <- rbind(tubble.df, tubble.df.tmp %>% filter(!is.na(allele_count))  )
}

tubble.df <- tubble.df %>% mutate(allele_freq = allele_count/chroms_avail) %>% as_tibble()
...
ggplot(tubble.df) + geom_histogram(aes(x=chroms_avail), bins = max(tubble.df$chroms_avail)/5) +theme_clear() + scale_x_continuous( breaks = seq(1,max(tubble.df$chroms_avail),5)) + scale_y_log10()
...
ggplot(tubble.df) + geom_histogram(aes(x=var_count), bins = max(tubble.df$var_count))+theme_clear() + scale_x_continuous( breaks = seq(1,max(tubble.df$var_count)+1,1))
...
```{r echo=FALSE}
ggplot(tubble.df %>%  group_by(site_id) %>% summarize(maf = min(allele_freq), vc = as.factor(max(var_count))) ) + geom_freqpoly(aes(x=maf, group=vc, color=vc))
```


```{r echo=FALSE}
ggplot(tubble.df %>%  group_by(site_id) %>% summarize(mac = min(allele_count) )) + geom_histogram(aes(x=mac))
```

Let's start with the simplest case: biallelic sites with all 60 chromosomes available and allele count of 1 (ie, one het indiv.)
hmmm, a number of MNPs showing up; let's try to filter those out for a really simple case. 
...
vcftools --vcf variants/all_samples.vs_dm6.bwaUniq.vcf --keep-only-indels --recode --recode-INFO-all --stdout | grep -v "#" | grep -v "TYPE=mnp" | grep -v "TYPE=complex" >> variants/all_samples.vs_dm6.bwaUniq.vcf.noMNP.tmp



vcftools --vcf variants/all_samples.vs_dm6.bwaUniq.vcf.noMNP.tmp --keep-only-indels --counts --out indelCount.test
...
tubble.df %>% filter(var_count == 2) %>% filter(chroms_avail==60) %>% filter(allele_count==1) %>% nrow()
```
...
cat indelCount.test.frq.count  | tail -n +2 | tr ":" "\t" | nl -n ln | grep -v Un | grep -v rand  > dev/indelCount.test.frq.count
...
```{r echo=FALSE}
tubble.df.biallele <- tubble.df %>% filter(var_count == 2) %>% filter(chroms_avail==60) %>% gather(variable, value, -(site_id:chroms_avail), -var_num) %>% unite(temp, var_num, variable) %>%  spread(temp, value) %>% mutate(len1 = nchar(`1_allele`),len2 = nchar(`2_allele`))
```
...
tubble.df.biallele.tmp <- tubble.df.biallele %>% filter(`1_allele_count` >= `2_allele_count`) %>% mutate(delta_bp=len2-len1,  mac=`2_allele_count`)

tubble.df.biallele <- rbind(tubble.df.biallele.tmp, tubble.df.biallele %>% filter(`1_allele_count` < `2_allele_count`) %>% mutate(delta_bp=len1-len2, mac=`1_allele_count`))
...
ggplot(tubble.df.biallele) + geom_point(aes(x=mac, y=delta_bp, color=chrom))

#ggplot(tubble.df.biallele) + geom_point(aes(x=mac, y=delta_bp, color=chrom), position='jitter', alpha=0.8) + theme_clear() + theme_void()  + theme(legend.position="none", plot.background = element_rect(fill = 'black', colour = 'black'))
```
...
cat variants/all_samples.vs_dm6.bwaUniq.vcf | ~/modules/vcflib/bin/vcfallelicprimitives | vcftools --vcf - --keep-only-indels  --recode --recode-INFO-all --stdout  > test.IndelsOnly.primSplit.vcf
```

hmmm yes, this seems right.


maybe add some depth-filtering to improve genotype calls? avg depth is pretty high so it could be set stringent. 
The --minDP 10  flag in vcftools will do this (sets the called genotype to ./.; --counts then tallies this GT as uncalled.)
...
cat variants/all_samples.vs_dm6.bwaUniq.vcf | vcftools --vcf -  --minDP 10 --recode --recode-INFO-all --stdout | ~/modules/vcflib/bin/vcfallelicprimitives | vcftools --vcf - --keep-only-indels --recode-INFO-all --stdout | vcftools --vcf -  --counts --out tomato.count 
```
...
Hmmm, looks like vcfallelicprimatives breaks up MNPs and such, possibly unwanted. Maybe shunt the MNPs down a different path? "complex" repeats it doesn't seem to touch.
...
cat variants/all_samples.vs_dm6.bwaUniq.vcf | vcftools --vcf -  --minDP 10 --recode --recode-INFO-all --stdout | ~/modules/vcflib/bin/vcfallelicprimitives | vcftools --vcf - --keep-only-indels --recode-INFO-all --stdout | grep -v "TYPE=complex"| vcftools --vcf -  --counts --out tomato.count 
```


```{bash eval=FALSE}
cat tomato.count.frq.count | tail -n +2 | tr ":" "\t" | nl -n ln | grep -v Un | grep -v rand  > dev/tomato.count.frq.count
```


```{r echo=FALSE}

freak_count_loader <- function(filename){
...
primitive_tubby <- freak_count_loader("tomato.count.frq.count") 

primitive_tubby.biallele <- primitive_tubby %>% filter(var_count == 2) %>% filter(chroms_avail==60) %>% gather(variable, value, -(site_id:chroms_avail), -var_num) %>% unite(temp, var_num, variable) %>%  spread(temp, value) %>% mutate(len1 = nchar(`1_allele`),len2 = nchar(`2_allele`), `1_allele_count`=as.numeric(`1_allele_count`), `2_allele_count`=as.numeric(`2_allele_count`), `1_allele_freq`=as.numeric(`1_allele_freq`), `2_allele_freq`=as.numeric(`2_allele_freq`)) 


primitive_tubby.biallele.tmp <- primitive_tubby.biallele %>% filter(`1_allele_count` >= `2_allele_count`) %>% mutate(delta_bp=len2-len1,  mac=`2_allele_count`)

primitive_tubby.biallele <- rbind(primitive_tubby.biallele.tmp, primitive_tubby.biallele %>% filter(`1_allele_count` < `2_allele_count`) %>% mutate(delta_bp=len1-len2, mac=`1_allele_count`))

ggplot(primitive_tubby.biallele) + geom_point(aes(x=mac, y=delta_bp, color=chrom))
```
...
```{r echo=FALSE}
ggplot(primitive_tubby.biallele %>% filter(chrom != "chrY") %>%  filter(chrom != "chrM")) + geom_freqpoly(aes(x=mac, color=chrom), bins=31)
```

Getting some weird errors on the proposed pipeline so going through stepwise:

```{bash eval=FALSE}
cat variants/all_samples.vs_dm6.bwaUniq.vcf | vcftools --vcf -  --minDP 10 --recode --recode-INFO-all --out test.depthFiltered

 cat test.depthFiltered.recode.vcf | grep -v "TYPE=complex" | ~/modules/vcflib/bin/vcfallelicprimitives  >  test.depthFiltered.simplified.primitive.vcf
```

hmm it looks like the sites which are broken  down by vcfallelicprimitives are in a different format than the SNPs. 

Also, vcfallelicprimitiives reclassifies some variants as complex so might as well exclude them afterwards....

```{bash eval=FALSE}
vcftools --vcf test.depthFiltered.simplified.primitive.vcf --keep-only-indels --recode --recode-INFO-all  --stdout > test.depthFiltered.simplified.primitive.indelOnly.vcf 

vcftools --vcf test.depthFiltered.simplified.primitive.indelOnly.vcf   --counts --out test.depthFiltered.simplified.primitive.indelOnly.count 

cat test.depthFiltered.simplified.primitive.indelOnly.count.frq.count | tail -n +2 | tr ":" "\t" | nl -n ln | grep -v Un | grep -v rand  > dev/test.depthFiltered.simplified.primitive.indelOnly.count.frq.count
...
primitive_tubby <- freak_count_loader("test.depthFiltered.simplified.primitive.indelOnly.count.frq.count") 
...
ggplot(primitive_tubby.biallele) + geom_point(aes(x=mac, y=delta_bp, color=chrom), position='jitter')
...
cat misc/dannysCalls.prebed | cut -f 2 -d " " | tr -d "," | tr ":" "\t"  | awk '{print $1, $2-100, $2+100}' | tr " " "\t" > misc/dannysCalls.bed 

edtools intersect -wa -wb -a misc/dannysCalls.bed -b test.depthFiltered.simplified.primitive.indelOnly.vcf 
...
Nice! adds a help rule to a snakefile:
https://www.biostars.org/p/220268/

```{r echo=FALSE}

primitive_tubby <- freak_count_loader("analysis/alleleCounts.simpleIndels.dpthFilt.biallelic.universal.out") 


primitive_tubby.biallele <- primitive_tubby %>% gather(variable, value, -(site_id:chroms_avail), -var_num) %>% unite(temp, var_num, variable) %>%  spread(temp, value) %>% mutate(len1 = nchar(`1_allele`),len2 = nchar(`2_allele`), `1_allele_count`=as.numeric(`1_allele_count`), `2_allele_count`=as.numeric(`2_allele_count`), `1_allele_freq`=as.numeric(`1_allele_freq`), `2_allele_freq`=as.numeric(`2_allele_freq`)) 
...
total number of sites: $`r primitive_tubby.biallele %>% nrow()  `$
```{r echo=FALSE}
primitive_tubby.biallele %>% group_by(chrom) %>% summarize(count=n())
```

Logarithmic color scale in legend:
https://stackoverflow.com/questions/8069837/is-there-a-built-in-way-to-do-a-logarithmic-color-scale-in-ggplot2

```{r echo=FALSE}
ggplot(primitive_tubby.biallele %>%  group_by(mac, delta_bp) %>% summarise(count=n())) + geom_tile(aes(x=mac,y=delta_bp, fill=count))  + scale_fill_gradient(name = "number sites", trans = "log", breaks = c(1,10,100,1000), labels = c(1,10,100,1000))
```

Thought about a ggMarginal histogram but it wasn't doing much for me

```{r echo=FALSE}
ggplot(primitive_tubby.biallele) + geom_freqpoly(aes(x=delta_bp,stat(density)), bins=13) + geom_freqpoly(data=primitive_tubby.biallele %>% filter(mac==1), aes(x=delta_bp,stat(density)), bins=13, color='red')
```

INDEL size & type profile similar between the singletons and the sites at large.

## 20 March 2019

OKAY so it turns out that the reads I have been using are actually Talia's mcm5 mutants which explains some things I guess. 

Updating the YAML to reflect this; they are now in the subgroup "mutants". 

The flies from [@Miller2016] are on NCBI as PRJNA285112 (isogenized parental flies, w1118 and Canton S) and PRJNA307070 (~200 individual male offspring). Since these are parent/offspring comparisons, maybe add a new tag (pedigree: parent/child)?

Per NCBI, the mating was "mother w5 and isogenic w[1118] father"

Hmm, looks like the offspring were run on 4 lanes and thus 4 different SRAs will need to be downloaded and merged per sample :(

Uh.... fastq-dump says 

>fastq-dump --split-3 SRR2044310
Rejected 431205 READS because of filtering out non-biological READS
Read 25541271 spots for SRR2044310
Written 25541271 spots for SRR2044310

... which is great and everything but I didn't set --skip-technical or --split-spot? Are they automatically invoked by --split-3?????
>Filters for individual reads       Applied only with --split-spot set
  --skip-technical                 Dump only biological reads 

Anyway the outcome is three files, two of paired reads and one of unpaired:
>$ ls
SRR2044310_1.fastq  SRR2044310_2.fastq  SRR2044310.fastq

## 21 March 2019

All of the DM flies are males except for one of the parents. What about Talia's flies? Looking at the breadth of coverage of the chrY, the median is ~30% covered, with two exceptions ~2%. By contrast, chrX was consistently ~90% covered. Talia says they are all Males.

The DM flies were subsampled down to 28 for a balanced comparison. 

UGHHHH adding the SRA tags gives each of up to four SRAs its own line!!! 
```{r echo=FALSE}

data_sets.df %>% select(-c(sra)) %>% filter(subgroups == 'all') %>% distinct() 
...
What does this mean as far as sample subset choice????

Need to rewrite the joint VCF rule to call variants by group; can always summon the whole-sample joint call via the "all" group

Will the VCF comparison & contrast section be at all relevant when the two experimental treatments are unrelated? 
(If I'm using BWA-Uniq, I'll still want to compare to BWA, even if I ultimately don't use the BWA alignment for variant calling)

Also, what about using parental variants to validate/mask child variants?

## 22 March 2019

>can you look at the indel frequency distribution without chromosome 3 for mcm21-28 samples?  do not include mcm5-A7 and DfMcm5.

To answer this: generalize the current VCF_winnower rule, with preprocessing the input vcf to restrict the sample?

```{bash eval=FALSE}

cp variants/mutant.vs_dm6.bwaUniq.vcf variants/potato.vs_dm6.bwaUniq.vcf

snakemake -n -p analysis/potato.alleleCounts.simpleIndels.dpthFilt.biallelic.universal.out
```
>Building DAG of jobs...
InputFunctionException in line 250 of /proj/cdjones_lab/csoeder/EJgrepper/Snakefile:
KeyError: 'potato'

Hmm, looks like snakemake actually needs it to be a listed group in the config.yaml. But will it accept the extant variants/potato.vs_dm6.bwaUniq.vcf or try to force an new calling for the new group?

Using an updated config file:
```{bash eval=FALSE}

keepers=$(for i in {1..8}; do echo --indv mcm5-2"$i"; done  | tr "\n" " ")

vcftools --vcf variants/mutant.vs_dm6.bwaUniq.vcf $keepers --recode --recode-INFO-all --stdout >  variants/potato.vs_dm6.bwaUniq.vcf
 
snakemake  --configfile config.tmp.yaml -n -p analysis/potato.alleleCounts.simpleIndels.dpthFilt.biallelic.universal.out 
```

Works... well actually no the sample number filter is currently hardcoded into the Winnower rule. But copypasting the snakemake verboss output and modding (also means we can filter the chroms now rather than later)

```{bash eval=FALSE}
vcftools --chr chr2L --chr chr2R --chr chrX --vcf variants/potato.vs_dm6.bwaUniq.vcf --minDP 10 --recode --recode-INFO-all --stdout | grep -v "TYPE=complex" | vcfallelicprimitives | vcftools --vcf - --keep-only-indels --counts --stdout | awk '{if($3==2)print}' | awk '{if($4==16)print}' | tr ":" "\t" | nl -n ln  > analysis/potato.alleleCounts.simpleIndels.dpthFilt.biallelic.universal.out
...
taliaSubset <- freak_count_loader("analysis/potato.alleleCounts.simpleIndels.dpthFilt.biallelic.universal.out") 


taliaSubset.biallele <- taliaSubset %>% gather(variable, value, -(site_id:chroms_avail), -var_num) %>% unite(temp, var_num, variable) %>%  spread(temp, value) %>% mutate(len1 = nchar(`1_allele`),len2 = nchar(`2_allele`), `1_allele_count`=as.numeric(`1_allele_count`), `2_allele_count`=as.numeric(`2_allele_count`), `1_allele_freq`=as.numeric(`1_allele_freq`), `2_allele_freq`=as.numeric(`2_allele_freq`)) 


taliaSubset.biallele.tmp <- taliaSubset.biallele %>% filter(`1_allele_count` >= `2_allele_count`) %>% mutate(delta_bp=len2-len1,  mac=`2_allele_count`)

taliaSubset.biallele <- rbind(taliaSubset.biallele.tmp, taliaSubset.biallele %>% filter(`1_allele_count` < `2_allele_count`) %>% mutate(delta_bp=len1-len2, mac=`1_allele_count`))
```


Filtering as before, there are  $`r taliaSubset.biallele %>% nrow()  `$ sites:


```{r echo=FALSE}
taliaSubset.biallele %>% group_by(chrom) %>% summarize(count=n()) %>%  kable(caption="Quality Biallaeleic INDELs per Chromosome")
```


```{r echo=FALSE}
ggplot(taliaSubset.biallele %>%  group_by(mac, delta_bp) %>% summarise(count=n())) + geom_tile(aes(x=mac,y=delta_bp, fill=count)) + theme_clear() + labs(x="Minor Allele Count (chromosomes)", y="Nucleotides Inserted or Deleted", title="Heatmap of INDEL Count, by Frequency and Size") + scale_fill_gradient(name = "Site Count", trans = "log", breaks = c(1,10,100,1000), labels = c(1,10,100,1000)) + theme( panel.background = element_rect(fill = "black", colour = "black"))  + scale_x_continuous(breaks=seq(0,8, 1))
```

The sudden appearance of variants with a MAC of 0 is because when the starting VCF is subsetted by individual, variant sites are retained even if all subsetted individuals are 0/0. 
Kicking out all sites with an alternate allele count of 0....

```{r echo=FALSE}
taliaSubset.biallele.someAlt <- taliaSubset.biallele %>% filter(`2_allele_count` > 0 ) 
```

With non-variable sites removed there are $`r taliaSubset.biallele.someAlt %>% nrow()  `$ sites remaining:


```{r echo=FALSE}
taliaSubset.biallele.someAlt %>% group_by(chrom) %>% summarize(count=n()) %>%  kable(caption="Quality Biallaeleic INDELs per Chromosome")
```


```{r echo=FALSE}
ggplot(taliaSubset.biallele.someAlt %>%  group_by(mac, delta_bp) %>% summarise(count=n())) + geom_tile(aes(x=mac,y=delta_bp, fill=count)) + theme_clear() + labs(x="Minor Allele Count (chromosomes)", y="Nucleotides Inserted or Deleted", title="Heatmap of INDEL Count, by Frequency and Size") + scale_fill_gradient(name = "Site Count", trans = "log", breaks = c(1,10,100,1000), labels = c(1,10,100,1000)) + theme( panel.background = element_rect(fill = "black", colour = "black"))  + scale_x_continuous(breaks=seq(0,8, 1))
```


```{r echo=FALSE}
ggplot(taliaSubset.biallele.someAlt) + geom_freqpoly(aes(x=mac),bins=6, color='black') + geom_freqpoly(aes(x=mac, color=chrom),bins=9)  + theme_clear() + labs(x="Minor Allele Count (chromosomes)", y="Number of Sites", title="Number of Quality INDELs, by Minor Allele Count and Chromosome")  + scale_x_continuous(breaks=seq(0,8, 1))
```


```{r echo=FALSE}
ggplot(taliaSubset.biallele.someAlt) + geom_freqpoly(aes(x=mac, stat(density)),bins=6, color='black') + geom_freqpoly(aes(x=mac, color=chrom, stat(density)),bins=9)  + theme_clear() + labs(x="Minor Allele Count (chromosomes)", y="Site Density", title="Density of Quality INDELs, by Minor Allele Count and Chromosome")  + scale_x_continuous(breaks=seq(0,8, 1))
```



```{r echo=FALSE}
ggplot(taliaSubset.biallele.someAlt)  + geom_freqpoly(aes(x=delta_bp), bins=13) + geom_freqpoly(aes(x=delta_bp, color=chrom), bins=13) + theme_clear() + labs(x="Nucleotides Inserted/Deleted", y="Number of Sites", title="INDEL Count by Size")
```
...
```{r echo=FALSE}
taliaSubset.biallele.someAlt %>% filter(mac==1) %>% group_by(chrom) %>% summarise(count=n())  %>% kable(caption="Distribution of Singleton INDELs")
```

But, I'm not sure why there are singletons appearing on the X? Being males there is only one chrX so it would be read as homozygous at all points (and, the MAC for chrX is exaggerated by 2x). 

Looking at the allele counts mod 2 will give a lower bound to the number of sites on chrX with a heterozygote called (it won't reveal cases in which there are an even number of heterozygotes)


```{r echo=FALSE}

taliaSubset.biallele.someAlt %>% filter(chrom=="chrX") %>% mutate(mod = mac %% 2) %>% group_by(mod) %>% summarise(count=n())
```


Using the --het flag with vcftools to take another look??

```{bash eval=FALSE}
vcftools --chr chrX --vcf variants/potato.vs_dm6.bwaUniq.vcf --minDP 10 --recode --recode-INFO-all --stdout | grep -v "TYPE=complex" | vcfallelicprimitives | vcftools --vcf - --keep-only-indels --het --out taliaSubset
```
